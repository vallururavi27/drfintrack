...
if (!supabaseKey) {
  ...
  console.error('For Windows PowerShell: $env:SUPABASE_SERVICE_ROLE_KEY="your_key_here"');
  console.error('For Windows Command Prompt: set SUPABASE_SERVICE_ROLE_KEY=your_key_here');
  console.error('For Linux/Mac: export SUPABASE_SERVICE_ROLE_KEY="your_key_here"');
  process.exit(1);
}
    ...
    for (const table of tables) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (batchCount >= batchSize || i === data.length - 1) {
          try {
            await batch.commit();
            console.log(`Committed batch ${batchNumber} of ${batchCount} records for ${table}`);
            batchNumber++;
            batchCount = 0;
          } catch (batchError) {
            console.error(`Error committing batch for ${table}:`, batchError);
          }
        }
      }
